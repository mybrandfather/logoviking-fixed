...
const tools: Tool[] = [
  ...
  { slug: "resize-image",           name: "Resize Image",            category: "image",   description: "Resize JPG, PNG, and WebP images to exact pixel dimensions in your browser while preserving aspect ratio." },
...
const toolSeoDetails: Record<string, ToolSeoDetail> = {
  "gif-to-video": {
    seoTitle: "GIF to WebM Converter — Convert GIF to WebM Online",
    h1: "GIF to WebM Converter",
    description: "Convert an animated GIF to WebM in supported browsers, download the video, and copy looping HTML. MP4 and ffmpeg guidance is included.",
    intro: "Convert an animated GIF to WebM directly in supported browsers. Choose a capture duration, download the WebM result, and copy looping video HTML for a website. MP4 and frame-perfect conversion guidance is included through ffmpeg.",
    sections: [
      { title: "Why convert GIF to WebM for a website?", paragraphs: ["Animated GIF is convenient, but it is inefficient for many full-color animations. WebM uses modern video compression and can often deliver the same visual loop with fewer transferred bytes.", "For decorative or repeating website motion, a muted autoplaying video with loop and playsinline can replace a GIF while keeping familiar animation behavior."] },
      { title: "Browser conversion and MP4 workflow", paragraphs: ["LogoViking uses the browser MediaRecorder API when WebM encoding is supported. The result is recorded for the duration you choose, so select a duration that captures the loop you need.", "If you need MP4 output, exact frame timing, or a browser that cannot encode WebM, use the ffmpeg commands shown in the tool."] }
    ],
    faqs: [
      { q: "Can I convert GIF to WebM online with LogoViking?", a: "Yes in browsers that support WebM recording through MediaRecorder. The tool records the animated GIF for the duration you select and provides a downloadable WebM file." },
      { q: "Is WebM usually smaller than GIF?", a: "For many full-color animations, modern video compression is more efficient than GIF. The actual reduction depends on the animation, dimensions, frame rate, and encoding settings." },
      { q: "Can the tool create MP4 too?", a: "The browser workflow produces WebM. The page also provides ffmpeg commands for MP4 or frame-accurate conversion." }
    ],
    relatedTools: ["compress-image", "image-optimizer", "aspect-ratio-calculator"],
    relatedBlogs: ["gif-to-webm-guide", "compress-images-for-web", "webp-vs-jpg-guide"]
  },
  "resize-image": {
    seoTitle: "Resize Image Online — Free Image Resizer",
    h1: "Resize Image Online",
    description: "Resize JPG, PNG, and WebP images to exact pixel dimensions in your browser while preserving aspect ratio.",
    intro: "Resize an image to exact pixel dimensions in your browser. Keep aspect ratio enabled to prevent stretching, then download the resized image for websites, marketplaces, email, or social media.",
    sections: [
      { title: "How to resize an image without distortion", paragraphs: ["Start from the original image dimensions, choose the width you actually need, and keep the aspect-ratio lock on unless you intentionally want a different shape.", "Downscaling usually preserves appearance well. Upscaling cannot create missing detail and may make a small source image look softer or pixelated."] },
      { title: "Resize before you compress for the web", paragraphs: ["A website rarely needs a 5000-pixel-wide photo if the image is displayed at 1200 pixels. Reducing dimensions first removes unnecessary pixels before compression.", "After resizing, use the Image Compressor or a WebP converter when smaller transfer size matters. Keep the original file as your master copy."] }
    ],
    faqs: [
      { q: "How do I resize an image without stretching it?", a: "Keep aspect ratio enabled and change one dimension. The matching dimension should scale proportionally." },
      { q: "Does resizing an image reduce quality?", a: "Downscaling is usually safe when the source is good. Enlarging a small image can soften details because resizing cannot recreate pixels that were never present." },
      { q: "Should I resize an image before compressing it?", a: "Yes when the original dimensions are larger than the final display size. Resizing first often produces a much smaller final web image." }
    ],
    relatedTools: ["compress-image", "image-dimensions", "aspect-ratio-calculator", "webp-converter"],
    relatedBlogs: ["compress-images-for-web", "etsy-image-size-guide", "aspect-ratio-guide"]
  },
  "gradient-generator": {
    seoTitle: "Color Gradient Generator — CSS Linear & Radial Gradients",
    h1: "Color Gradient Generator",
    description: "Create linear and radial CSS color gradients, preview them live, adjust direction or angle, and copy the CSS.",
    intro: "Create a CSS color gradient visually, preview it as you edit, and copy the ready-to-use background code. Choose two colors, switch between linear and radial gradients, and set a preset direction or custom angle.",
    sections: [
      { title: "Create a CSS gradient visually", paragraphs: ["Pick the two colors you want to blend and choose a linear or radial gradient. The preview updates immediately so you can judge the transition before copying the CSS.", "For linear gradients, use a preset direction or enter a custom angle from 0 to 360 degrees."] },
      { title: "Linear gradients vs radial gradients", paragraphs: ["Linear gradients blend colors along a line and work well for buttons, hero backgrounds, cards, and overlays. Radial gradients spread outward from a center point and can create spotlight or glow effects.", "Copy the generated CSS into a background property, then test text contrast and readability on top of the gradient in the final design."] }
    ],
    faqs: [
      { q: "What CSS does the color gradient generator create?", a: "It creates a CSS background declaration using linear-gradient() or radial-gradient() with the colors and direction you choose." },
      { q: "Can I set a custom gradient angle?", a: "Yes. For linear gradients you can choose a preset direction or enter a custom angle between 0 and 360 degrees." },
      { q: "Can I copy the gradient CSS?", a: "Yes. The tool shows the generated background code and provides a copy button." }
    ],
    relatedTools: ["color-palette-generator", "color-picker", "font-pair-generator"],
    relatedBlogs: ["aspect-ratio-guide", "youtube-thumbnail-secrets"]
  },
  "logo-size-generator": {
    seoTitle: "Logo Size Generator — Logo Dimensions for Social & Web",
    h1: "Logo Size Generator & Guide",
    description: "Find logo, profile, banner, favicon, and social image dimensions for popular platforms and copy the size list.",
    intro: "Look up practical logo and brand-image dimensions for websites and popular social platforms. Filter by platform, compare common use cases, and copy the size list for your design handoff.",
    sections: [
      { title: "Start with a flexible master logo", paragraphs: ["Keep a high-resolution master logo, ideally in a vector format when available, then export platform-specific raster copies from that source. Avoid repeatedly resizing a small downloaded logo upward.", "Square profile images, wide banners, website favicons, and social preview images all use different aspect ratios, so one exported file rarely fits every destination."] },
      { title: "Logo size is not the same as display size", paragraphs: ["Platforms often resize uploaded branding assets for different screens. Prepare the recommended upload dimensions, keep important artwork inside safe areas, and preview the result after upload.", "For website icons, use the Favicon Generator. For proportional resizing, use Resize Image or the Aspect Ratio Calculator before exporting."] }
    ],
    faqs: [
      { q: "What size should a logo be?", a: "There is no single universal logo size. The correct dimensions depend on whether the logo is used as a profile image, banner, website header, favicon, email logo, or social preview." },
      { q: "Should I keep a vector version of my logo?", a: "Yes when possible. A vector master can be exported cleanly at many raster sizes without repeatedly enlarging a small bitmap." },
      { q: "What size should a website favicon be?", a: "Common favicon outputs include 16×16 and 32×32 pixels, with larger app-icon sizes used for touch icons and manifests." }
    ],
    relatedTools: ["favicon-generator", "social-media-size-generator", "resize-image", "aspect-ratio-calculator"],
    relatedBlogs: ["favicon-size-guide", "etsy-image-size-guide", "aspect-ratio-guide"]
  }
};
...
const blogPosts: BlogPost[] = [
  ...
  { slug:"png-vs-jpg-explained",title:"PNG vs JPG: Which Image Format Should You Use?",excerpt:"A practical PNG vs JPG comparison for websites, screenshots, photos, transparency, and file size.",category:"Image Tools",readingTime:"6 min",updatedAt:"2026-08-14",sections:[{id:"difference",title:"PNG and JPG solve different problems",paragraphs:["JPG is designed for photographs and continuous-tone images where smaller files matter more than perfect pixel preservation. PNG is lossless and supports transparency, which makes it useful for graphics, screenshots, logos, and interface assets.","Neither format is always better. Choose based on the image content and where it will be used."]},{id:"web",title:"Which is better for a website?",paragraphs:["For photographs, JPG or WebP often produces smaller files. For transparent graphics, PNG or WebP is usually more appropriate.","Before publishing, resize the image to the dimensions you actually need and compress it. LogoViking includes separate PNG-to-JPG, JPG-to-PNG, WebP conversion, resize, and compression tools."]}],faqs:[{question:"Is PNG higher quality than JPG?",answer:"PNG uses lossless compression, so repeated saves do not introduce JPEG-style compression artifacts. JPG can still look excellent for photos at sensible quality settings."},{question:"Does JPG support transparency?",answer:"No. Standard JPG does not support transparent backgrounds. Use PNG or WebP when transparency is required."}],related:["webp-vs-jpg-guide","compress-images-for-web","favicon-size-guide"]},
  {
    ...
    title:"How to Compress Images for Websites Without Ruining Quality",
    seoTitle:"Compress Images for Websites — JPG, PNG & WebP Guide",
    h1:"How to Compress Images for Websites Without Ruining Quality",
    excerpt:"Learn how to compress images for websites: resize oversized files, choose JPG, PNG, or WebP, tune quality, and check the result before publishing.",
    category:"Image Tools",readingTime:"8 min",updatedAt:"2026-08-27",
    sections:[
      {id:"why",title:"Why image compression matters for websites",paragraphs:["Large image files increase page weight and can make pages feel slow, especially on mobile connections. The goal is not the smallest possible file at any cost; it is the smallest file that still looks good at the dimensions visitors will actually see.","Keep an original master copy, then create an optimized delivery version for the website."]},
      {id:"resize-first",title:"Resize oversized images before compressing",paragraphs:["If a photo is 5000 pixels wide but the page displays it near 1200 pixels, resize it first. Removing unused pixels usually saves more than repeatedly lowering quality on the oversized original.","Check the target layout, resize to the needed dimensions, then compress the resized copy."]},
      {id:"formats",title:"Choose JPG, PNG, or WebP for the job",paragraphs:["JPG works well for photographs when transparency is not required. PNG is useful for screenshots, line art, and transparent graphics. WebP is a practical web-delivery format when you want modern compression with broad current-browser support.","Do not convert automatically just because a format is newer. Compare the image at its final display size and choose the format that gives an acceptable visual result at a useful file size."]},
      {id:"workflow",title:"A practical website image-compression workflow",paragraphs:["Check the original dimensions, resize if necessary, compress the delivery copy, and compare the before-and-after file sizes. Inspect text, faces, gradients, and sharp edges at normal viewing size before publishing.","LogoViking includes an Image Compressor, Image Dimensions Checker, Image Resizer, and JPG/PNG-to-WebP tools so those steps can stay in one workflow."]}
    ],
    faqs:[{question:"How much should I compress images for a website?",answer:"Use the lowest file size that still looks good at the final display size. There is no single quality percentage or kilobyte target that works for every photo, graphic, or layout."},{question:"Should I resize images before compressing them?",answer:"Yes when the source dimensions are larger than the page needs. Resizing first removes unnecessary pixels and usually improves the final size-to-quality balance."},{question:"Is WebP always better than JPG or PNG?",answer:"No. WebP is often efficient for web delivery, but the best choice depends on transparency, image content, workflow compatibility, and the visual result after compression."}],
    related:["webp-vs-jpg-guide","png-vs-jpg-explained","gif-to-webm-guide"],
    relatedTools:["compress-image","resize-image","image-dimensions","jpg-to-webp","png-to-webp"]
  },
  {
    slug:"etsy-image-size-guide",
    title:"Etsy Image Sizes: Listing Photos, Banners & Shop Graphics",
    seoTitle:"Etsy Image Sizes — Listing Photos & Shop Banners",
    h1:"Etsy Image Sizes: Listing Photos, Banners & Shop Graphics",
    excerpt:"Current Etsy image-size guidance for listing photos, shop banners, and seller graphics, plus practical resize and compression steps before upload.",
    category:"Image Tools",readingTime:"8 min",updatedAt:"2026-08-27",
    sections:[
      {id:"listing-images",title:"Etsy listing image size basics",paragraphs:["Etsy currently recommends listing photos with both width and height of at least 2000 pixels. Etsy also says the first listing photo should be at least 635 pixels in both dimensions to avoid a possible search-visibility disadvantage.","For the first listing image, Etsy recommends a horizontal or square orientation so the focal point survives thumbnail crops. Using a consistent shape across listing photos also creates a more cohesive gallery."]},
      {id:"banners",title:"Etsy shop banner sizes",paragraphs:["For standard shop customization, Etsy lists a minimum size of 1200×160 pixels for a Mini Banner and 1200×300 pixels for a Big Banner. Etsy Plus carousel banner images also use a 1200×300 pixel minimum.","Prepare important text and logos with generous safe space because banners may be displayed at different sizes on desktop and mobile."]},
      {id:"formats",title:"Etsy image formats and transparency",paragraphs:["Etsy supports JPG, GIF, PNG, SVG, and HEIC shop images. Etsy notes that animated GIF files and transparent PNG files are not supported as expected; transparent areas can appear black after upload.","For product photography, JPG is a straightforward choice. For graphics that need transparency elsewhere, keep a transparent master copy and export an Etsy-ready version with a solid background when necessary."]},
      {id:"prepare",title:"Resize and compress Etsy images before upload",paragraphs:["Start with a clean master image, crop for the listing composition, resize to the dimensions you need, and then compress the upload copy without making product details hard to inspect.","If uploads are slow, reduce unnecessary file weight rather than shrinking a high-quality product image below Etsy's recommended dimensions. Use the Image Dimensions Checker before resizing so you know the exact starting size."]}
    ],
    faqs:[{question:"What size should Etsy listing photos be?",answer:"Etsy currently recommends listing photos with both width and height of at least 2000 pixels. The first listing image should be at least 635 pixels in both dimensions."},{question:"Should the first Etsy listing photo be square or landscape?",answer:"Etsy recommends the first listing photo be horizontal or square so the main subject is less likely to be cut off in thumbnail crops."},{question:"Does Etsy support WebP listing images?",answer:"Etsy's current supported shop-image list includes JPG, GIF, PNG, SVG, and HEIC. WebP is not included in that list, so use a supported format for Etsy uploads."},{question:"Can I upload a transparent PNG to Etsy?",answer:"Etsy says transparent PNG files are not supported as transparent images; transparent areas can appear black. Export a version with an intentional background when needed."}],
    related:["compress-images-for-web","png-vs-jpg-explained","aspect-ratio-guide"],
    relatedTools:["resize-image","compress-image","image-dimensions","aspect-ratio-calculator"]
  },
...
function ToolPage(){
  ...
  const meta=categoryMeta[tool.category];
  const Icon=meta.icon;
  const seo=toolSeoDetails[tool.slug];
  const related=seo?.relatedTools.map(getToolBySlug).filter((t):t is Tool=>Boolean(t))??tools.filter(t=>t.slug!==tool.slug&&t.category===tool.category).slice(0,5);
  const blogs=seo?.relatedBlogs.map(getBlogBySlug).filter((b):b is BlogPost=>Boolean(b))??blogPosts.slice(0,3);
  const faqs=seo?.faqs??faqGroups.flatMap(g=>g.questions).slice(0,5);
  const applicationSchema={"@type":"WebApplication","name":seo?.h1??tool.name,"url":`${siteDomain}/tools/${tool.slug}`,"description":seo?.description??tool.description,"applicationCategory":tool.category==="image"||tool.category==="designer"?"DesignApplication":"UtilitiesApplication","operatingSystem":"Any","offers":{"@type":"Offer","price":"0","priceCurrency":"USD"}};
  const faqSchema=seo?.faqs.length?{"@type":"FAQPage","mainEntity":seo.faqs.map(f=>({"@type":"Question","name":f.q,"acceptedAnswer":{"@type":"Answer","text":f.a}}))}:null;
  const toolSchema={"@context":"https://schema.org","@graph":[applicationSchema,...(faqSchema?[faqSchema]:[])]};
  return(
    <>
      <SeoHead title={seo?.seoTitle??tool.name} description={seo?.description??tool.description} schema={toolSchema}/>
      <PageWrap>
        <Breadcrumb items={[{label:"Home",href:"/"},{label:"Tools",href:"/tools"},{label:meta.name,href:`/categories/${tool.category}`},{label:seo?.h1??tool.name}]}/>
        <div className="mb-6 flex flex-wrap items-center gap-4">
          ...
          <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white">{seo?.h1??tool.name}</h1><p className="text-sm text-gray-500 dark:text-gray-400">{seo?.description??tool.description}</p></div>
...
function ToolSeoContent({tool}:{tool:Tool}){
  const targeted=toolSeoDetails[tool.slug];
  ...
  const steps=targeted?[
    "Open the tool and provide the image, colors, or settings it needs.",
    "Adjust the available controls while reviewing the result.",
    "Copy or download the output and verify it in the destination platform."
  ]:fallback.steps;
  const faqs=targeted?.faqs??fallback.faqs;
  const relatedTools=targeted?.relatedTools.map(getToolBySlug).filter((t):t is Tool=>Boolean(t))??[];
  const relatedBlogs=targeted?.relatedBlogs.map(getBlogBySlug).filter((b):b is BlogPost=>Boolean(b))??[];
  return <section className="mt-10 space-y-6">
    <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
      <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900"><h2 className="text-xl font-bold text-gray-900 dark:text-white">How to use {targeted?.h1??tool.name}</h2><p className="mt-3 text-sm leading-7 text-gray-600 dark:text-gray-400">{targeted?.intro??fallback.intro}</p><ol className="mt-4 space-y-2">{steps.map((step,i)=><li key={step} className="flex gap-3 text-sm text-gray-600 dark:text-gray-400"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-violet-100 text-xs font-bold text-violet-700 dark:bg-violet-950 dark:text-violet-300">{i+1}</span><span>{step}</span></li>)}</ol><p className="mt-5 text-xs text-gray-400">Privacy note: browser-side tools are preferred where practical. If a tool depends on an external service, its interface or documentation should make that clear.</p></div>
      <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900"><h2 className="text-xl font-bold text-gray-900 dark:text-white">{targeted?.h1??tool.name} FAQ</h2><div className="mt-3 space-y-2">{faqs.map(f=><details key={f.q} className="rounded-xl border border-gray-100 dark:border-gray-800"><summary className="cursor-pointer px-3 py-3 text-sm font-semibold text-gray-800 dark:text-gray-200">{f.q}</summary><p className="px-3 pb-3 text-sm leading-6 text-gray-600 dark:text-gray-400">{f.a}</p></details>)}</div></div>
    </div>
    {targeted&&<div className="grid gap-6 lg:grid-cols-2">
      {targeted.sections.map(section=><section key={section.title} className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900"><h2 className="text-xl font-bold text-gray-900 dark:text-white">{section.title}</h2>{section.paragraphs.map(paragraph=><p key={paragraph} className="mt-3 text-sm leading-7 text-gray-600 dark:text-gray-400">{paragraph}</p>)}</section>)}
    </div>}
    {targeted&&<div className="rounded-2xl border border-violet-100 bg-violet-50/40 p-6 dark:border-violet-900 dark:bg-violet-950/10"><h2 className="text-lg font-bold text-gray-900 dark:text-white">Related LogoViking resources</h2><div className="mt-3 flex flex-wrap gap-2">{relatedTools.map(t=><Link key={t.slug} to={`/tools/${t.slug}`} className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:border-violet-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">{t.name}</Link>)}{relatedBlogs.map(b=><Link key={b.slug} to={`/blog/${b.slug}`} className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:border-violet-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">{b.title}</Link>)}<Link to={`/categories/${tool.category}`} className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:border-violet-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">Browse {categoryMeta[tool.category].name}</Link></div></div>}
  </section>;
}
...
function BlogPostPage(){
  ...
  const articleSchema={"@type":"BlogPosting","headline":post.h1??post.title,"description":post.excerpt,"dateModified":post.updatedAt,"publisher":{"@type":"Organization","name":siteName,"url":siteDomain},"mainEntityOfPage":{"@type":"WebPage","@id":`${siteDomain}/blog/${post.slug}`}};
  ...
  return(
    <>
      <SeoHead title={post.seoTitle??post.title} description={post.excerpt} schema={combinedSchema}/>
      <PageWrap>
        <Breadcrumb items={[{label:"Home",href:"/"},{label:"Blog",href:"/blog"},{label:post.h1??post.title}]}/>
        <div className="grid gap-8 xl:grid-cols-[1fr_280px]">
          <article className="space-y-8">
            <div>
              <Badge variant="violet">{post.category}</Badge>
              <h1 className="mt-3 text-3xl font-bold tracking-tight text-gray-900 dark:text-white">{post.h1??post.title}</h1>
            ...
            <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-900">
              <p className="text-xs font-semibold uppercase tracking-widest text-violet-600 dark:text-violet-400 mb-3">Related resources</p>
              {post.relatedTools?.length?<div className="mb-3 flex flex-wrap gap-2">{post.relatedTools.map(toolSlug=>{const t=getToolBySlug(toolSlug);return t?<Link key={toolSlug} to={`/tools/${toolSlug}`} className="rounded-xl border border-violet-100 bg-white px-3 py-2.5 text-sm font-medium text-violet-700 hover:border-violet-300 dark:border-violet-900 dark:bg-gray-800 dark:text-violet-300">{t.name}</Link>:null})}</div>:null}
